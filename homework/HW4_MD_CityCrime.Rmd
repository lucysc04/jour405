...
```{r}
md_cities_rates |>
  summarize(IQR_violent_2020 = IQR(violent_rate_2020, na.rm = TRUE))

```
The data confirms that a small group of cities account for the highest violent crime rates, while may towns report very low or even zero rates. The middle 50% of cities fall within a moderate range, with my 90th percentile calculation working out that only about 10 cities rank in the top 10% statewide. This supports my original angle for the lede that dramatic swings in sparser populations can distort perception, sine extreme rates are concentrated in a handful of places rather than spread statewide. 
