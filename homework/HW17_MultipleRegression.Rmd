...
The positive correlations with voter turnout from the matrix include: income, median age, % w/ bachelor’s degree, and % white.
Negatively correlated: % unemployed population Littl/no correlation: population

Some independent variables are also correlated with each other- income and education- suggesting it may be harder for the model to look at separate effects as the variables affect each other. 

Scatterplots: linear relationships with turnout shown in the areas of income, median age, education, % white.  Unemployment= negative linear. 
...
The model fits well explaining about 85% of turnout and is statistically significant.

Income, median age, and percent white are significant and positively related to turnout. Population, education, and unemployment are not significant.

Overall, it’s a strong model, but not all variables matter.
...
Model 3 performs the best with it having a higher value than the full model and model 2. However, the difference between the 3 models is very small, meaning there is no real significance.

Model 4 performs worse which suggests that removing too many important variables makes the model's accuracy worse.

Model 2 and 3 perform just as well as the full model so a reduced model is preferred because it is easier to understand.
...
# Model 4
model4 <- lm(turnout ~ income + pct_bachelors_higher + median_age, data = md_turnout)
...
This analysis shows which types of counties are likely to vote at higher rates. A journalist could use it to explain patterns in turnout. It could also help identify places with low turnout and guide reporting on barriers- it provides context to make election coverage more informative.
