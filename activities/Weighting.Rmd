...
Weighting adjusts the results to better reflect the real population= can shift percentages and change who appears to be leading.

2. Which demographic groups show the largest differences between weighted and unweighted results?

Groups that are underrepresented/ overrepresented. For example, younger voters or highly educated people show the biggest changes.

3. How might weighting influence reporting of this data?

Weighting can change the story journalists tell, affecting headlines and how trends are understood publicly.
...
0.23 = the respondent is overrepresented, so their response counts much less than one person. 
3.04=  underrepresented, so their response counts for about three people.

3. Why might some respondents be weighted more heavily than others?

Because some groups are underrepresented in the sample, so their responses are increased to reflect the real population.
...
Rarely/never voters show the biggest difference, specifically in the percentage saying they plan to vote.

2. How might these differences affect reporting on likely voter turnout?

It could lower the reported turnout, since weighting reduces enthusiasm from less reliable voters.
...
These differences exist because some groups like engaged voters are overrepresented, while less frequent voters are underrepresented, so weighting corrects that imbalance.
